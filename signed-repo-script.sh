...
update_sources_list() {
    ...
    echo "[signed-by=\"$current_directory/${kerray[0]}.gpg\"] \"$current_directory\"" | run_with_sudo tee -a /etc/apt/sources.list >/dev/null
    ...
    # Optional apt update
    read -p "Do you want to apt update now" update_confirmed
    print_green $(grep -E "$search_pattern" /etc/apt/sources.list)
